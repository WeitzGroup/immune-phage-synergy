% Plot time series of system with different phage adsorption rates
%(a) phage adsorption rate \phi=10^-9.5 (b) \phi=10^-10.5 and (c)
%\phi=10^-11.5
% This figure is same as Fig. 6 but with different initial phage dose P_0
% given by (a) 10^4 ml^-1, (b) 2*10^10 ml^-1 and (c) 10^12 ml^-1.

close all;
clear all;

%% Initialize parameters
TT=30; % Length of time series (h)
...
r_dev=[0.9 0.01]; % Deviation ratio of initial bacterial load from coexistence state (if exists)
pop0_sub=[[BP(1)*r_dev(1) 1e4 imm0];[BP(2)*r_dev(2) 2e10 imm0];[1e8 1e12 imm0]];
...
toc
    ...
    saveas(gcf,'fig9a.fig')
    print('fig9a.eps','-depsc')
    figure(2);
    h_leg=legend([hand(2,2) hand(2,1)],{'Phage','Bacteria'},'Location','Southeast');
    set(h_leg,'box','off','FontSize',leg_size);
    saveas(gcf,'fig9b.fig')
    print('fig9b.eps','-depsc')
    figure(3);
    h_leg=legend([hand(3,1) hand(3,2)],...
    {'Bacteria','Phage'},'Location','Southwest');
    set(h_leg,'box','off','FontSize',leg_size);
    saveas(gcf,'fig9c.fig')
    print('fig9c.eps','-depsc')